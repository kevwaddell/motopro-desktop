...
<div class="login<?php echo (function_exists('is_maintenance') && is_maintenance()) ? ' maintenance_active':''; ?>" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="lostpasswordform" id="lostpasswordform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'lostpassword' ); ?>" method="post">
		...
		<p class="submit new-pass-btn">
			<input type="submit" name="wp-submit" id="wp-submit<?php $template->the_instance(); ?>" class="btn btn-default btn-block" value="<?php esc_attr_e( 'Get New Password' ); ?>" />
